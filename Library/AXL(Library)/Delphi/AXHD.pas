...
const
    ...
    IPxySPDRead               = $0C;      // PGM-1 current SPEED DATA READ, 16bit, $0000         
    IPxyNoOperation_8C        = $8C;      // No operation                                      
    IPxySPDCMPRead            = $0D;      // PGM-1 current SPEED compare DATA READ, 16bit, $0000    
    IPxySPDCMPWrite           = $8D;      // PGM-1 current SPEED compare DATA WRITE                  
    ...
    IPxySWLMTCONRead          = $36;      // SOFT LIMIT setting READ, 3bit, $00                      
    IPxySWLMTCONWrite         = $B6;      // SOFT LIMIT setting WRITE                                 
    IPxyMSWLMTCOMPRead        = $37;      // -SOFT LIMIT compare register setting READ, 32bit, $80000000
    IPxyMSWLMTCOMPWrite       = $B7;      // -SOFT LIMIT compare register setting WRITE                  
    IPxyPSWLMTCOMPRead        = $38;      // +SOFT LIMIT compare register setting READ, 32bit, $7FFFFFFF
    IPxyPSWLMTCOMPWrite       = $B8;      // +SOFT LIMIT compare register setting WRITE                  
    IPxyTRGCONRead            = $39;      // TRIGGER MODE setting READ, 32bit, $00010000             
    IPxyTRGCONWrite           = $B9;      // TRIGGER MODE setting WRITE                               
    IPxyTRGCOMPRead           = $3A;      // TRIGGER compare data setting READ, 32bit, $00000000      
    IPxyTRGCOMPWrite          = $BA;      // TRIGGER compare data setting WRITE                        
    IPxyICMRead               = $3B;      // INTERNAL M-DATA setting READ, 32bit, $80000000          
    IPxyICMWrite              = $BB;      // INTERNAL M-DATA setting WRITE                            
    IPxyECMRead               = $3C;      // EXTERNAL M-DATA setting READ, 32bit, $80000000          
    IPxyECMWrite              = $BC;      // EXTERNAL M-DATA setting WRITE                            
    ...
    IPySCRCON1Read            = $40;      // Script action setting register-1 READ, 32bit, $00000000
    IPySCRCON1Write           = $C0;      // Script action setting register-1 WRITE
    IPySCRCON2Read            = $41;      // Script action setting register-2 READ, 32bit, $00000000
    IPySCRCON2Write           = $C1;      // Script action setting register-2 WRITE
    IPySCRCON3Read            = $42;      // Script action setting register-3 READ, 32bit, $00000000 
    IPySCRCON3Write           = $C2;      // Script action setting register-3 WRITE
    IPySCRCONQRead            = $43;      // Script action setting register-Queue READ, 32bit, $00000000
    IPySCRCONQWrite           = $C3;      // Script action setting register-Queue WRITE
    IPySCRDATA1Read           = $44;      // Script action data register-1 READ, 32bit, $00000000 
    IPySCRDATA1Write          = $C4;      // Script action data register-1 WRITE
    IPySCRDATA2Read           = $45;      // Script action data register-2 READ, 32bit, $00000000 
    IPySCRDATA2Write          = $C5;      // Script action data register-2 WRITE
    IPySCRDATA3Read           = $46;      // Script action data register-3 READ, 32bit, $00000000 
    IPySCRDATA3Write          = $C6;      // Script action data register-3 WRITE
    IPySCRDATAQRead           = $47;      // Script action data register-Queue READ, 32bit, $00000000 
    IPySCRDATAQWrite          = $C7;      // Script action data register-Queue WRITE
    IPyNoOperation_48         = $48;      // No operation
    IPySCRQCLR                = $C8;      // Script Queue clear
    IPySCRCQSIZERead          = $49;      // Script action setting Queue index READ, 4bit, $00
    IPyNoOperation_C9         = $C9;      // No operation
    IPySCRDQSIZERead          = $4A;      // Script action data Queue index READ, 4bit, $00
    IPyNoOperation_CA         = $CA;      // No operation
    IPySCRQFLAGRead           = $4B;      // Script Queue Full/Empty Flag READ, 4bit, $05
    IPyNoOperation_CB         = $CB;      // No operation
    IPySCRQSIZECONRead        = $4C;      // Script Queue size setting(0~13) READ, 16bit, $D0D0
    IPySCRQSIZECONWrite       = $CC;      // Script Queue size setting(0~13) WRITE
    IPySCRQSTATUSRead         = $4D;      // Script Queue status READ, 12bit, $005
    ...
    IPyCAPCON1Read            = $50;      // Caption action setting register-1 READ, 32bit, $00000000
    IPyCAPCON1Write           = $D0;      // Caption action setting register-1 WRITE
    IPyCAPCON2Read            = $51;      // Caption action setting register-2 READ, 32bit, $00000000
    IPyCAPCON2Write           = $D1;      // Caption action setting register-2 WRITE
    IPyCAPCON3Read            = $52;      // Caption action setting register-3 READ, 32bit, $00000000 
    IPyCAPCON3Write           = $D2;      // Caption action setting register-3 WRITE
    IPyCAPCONQRead            = $53;      // Caption action setting register-Queue READ, 32bit, $00000000
    IPyCAPCONQWrite           = $D3;      // Caption action setting register-Queue WRITE
    IPyCAPDATA1Read           = $54;      // Caption action setting register-1 READ, 32bit, $00000000 
    IPyNoOperation_D4         = $D4;      // No operation
    IPyCAPDATA2Read           = $55;      // Caption action data register-2 READ, 32bit, $00000000 
    IPyNoOperation_D5         = $D5;      // No operation
    IPyCAPDATA3Read           = $56;      // Caption action data register-3 READ, 32bit, $00000000 
    IPyNoOperation_D6         = $D6;      // No operation
    IPyCAPDATAQRead           = $57;      // Caption action data register-Queue READ, 32bit, $00000000 
    IPyNoOperation_D7         = $D7;      // No operation
    IPyNoOperation_58         = $58;      // No operation
    IPyCAPQCLR                = $D8;      // Caption Queue clear
    IPyCAPCQSIZERead          = $59;      // Caption action setting Queue index READ, 4bit, $00
    IPyNoOperation_D9         = $D9;      // No operation
    IPyCAPDQSIZERead          = $5A;      // Caption action data Queue index READ, 4bit, $00
    IPyNoOperation_DA         = $DA;      // No operation
    IPyCAPQFLAGRead           = $5B;      // Caption Queue Full/Empty Flag READ, 4bit, $05
    IPyNoOperation_DB         = $DB;      // No operation
    IPyCAPQSIZECONRead        = $5C;      // Caption Queue size setting(0~13) READ, 16bit, $D0D0
    IPyCAPQSIZECONWrite       = $DC;      // Caption Queue size setting(0~13) WRITE
    IPyCAPQSTATUSRead         = $5D;      // Caption Queue status READ, 12bit, $005
    ...
    IPxyEXFILTERDRead         = $6A;      // External sensor filter bandwidth setting DATA READ, 32bit, $00050005
    IPxyEXFILTERDWrite        = $EA;      // External sensor filter bandwidth setting DATA WRITE
    ...
    IPxyFUNCONRead            = $70;      // Chip function setting DATA READ,
    IPxyFUNCONWrite           = $F0;      // Chip function setting DATA WRITE
    ...
    IPxyEXCNTCLRRead          = $77;      // EXTERNAL COUNTER setting DATA READ, 9bit, $00
    IPxyEXCNTCLRWrite         = $F7;      // EXTERNAL COUNTER setting DATA WRITE
