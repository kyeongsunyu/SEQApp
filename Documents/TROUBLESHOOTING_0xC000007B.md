...
| 대상 | 아키텍처 | 크기 | 링크 타임스탬프 |
| --- | --- | --- | --- |
| `C:\WORK\SEQApp.exe` | x86 (14C) | | |
| `C:\WORK\AXL.dll` | **x64 (8664)** | 8,583,680 | 2025-07-24 |
| `C:\WORK\EzBasicAxl.dll` | **x64 (8664)** | 129,536 | 2025-12-10 |

`OutDir=C:\WORK\` 이 `Debug|Win32` 구성에만 걸려 있어서, 빌드는 32비트로 나가는데
장비 폴더에는 64비트 AXL 런타임이 놓여 있었다.
이후 **x64 를 표준 구성으로 정하고**, `OutDir=C:\WORK\` 를 `Debug|x64` 로 옮겼다.
Win32 구성은 기본 출력 폴더를 쓰므로 두 비트가 같은 폴더에서 섞이지 않는다.

## 리포지토리 사본 기준값 (크기만으로 비트 판별)

| 파일 | 32Bit (x86) | 64Bit (x64) |
| --- | --- | --- |
| `AXL.dll` | 7,342,080 | 8,583,680 |
| `EzBasicAxl.dll` | 147,456 | 129,536 |
